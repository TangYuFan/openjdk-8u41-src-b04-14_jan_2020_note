...
//    printf("【log】hotspot/src/share/vm/classfile/systemDictionary.cpp resolve_instance_class_or_null()\n");
  ...
  // 通过类名和类加载器计算hash值,再将hash值转换为index,再 dictionary() 中查找是否有这个index的类,类加载信息是服装城 Klass 对象的
  unsigned int d_hash = dictionary()->compute_hash(name, loader_data);
  int d_index = dictionary()->hash_to_index(d_hash);
...
//    printf("【log】hotspot/src/share/vm/classfile/systemDictionary.cpp load_instance_class() 从字典中没有找到时，需要对类进行加载\n");
  ...
  // 使用指定的类加载器加载，最终会调用java.lang.ClassLoader类中的loadClass()方法执行类加载,也就是
  else {
...
//    printf("【log】hotspot/src/share/vm/classfile/classLoader.cpp SystemDictionary::find_or_define_instance_class() symbol=%s\n",class_name->as_C_string());
...
void SystemDictionary::initialize(TRAPS) {
    ...
    printf("【log】hotspot/src/share/vm/classfile/systemDictionary SystemDictionary::initialize() 系统Klass字典初始化\n");
...
void SystemDictionary::initialize_preloaded_classes(TRAPS) {
    ...
    printf("【log】hotspot/src/share/vm/classfile/systemDictionary SystemDictionary::initialize_preloaded_classes() 基本类的预加载\n");
